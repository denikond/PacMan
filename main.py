...
def main():
    ...
    while running:
        ...
        if keys[pygame.K_w]:
            ...
            context["player"].current_image = get_next_img_packman("up")
        if keys[pygame.K_s]:
            context["player"].rect = context["player"].rect.move(0, player_speed)
            context["player"].current_image = get_next_img_packman("down")
        if keys[pygame.K_a]:
            context["player"].rect = context["player"].rect.move(-1 * player_speed, 0)
            context["player"].current_image = get_next_img_packman("left")
        if keys[pygame.K_d]:
            context["player"].rect = context["player"].rect.move(player_speed, 0)
            context["player"].current_image = get_next_img_packman("right")
